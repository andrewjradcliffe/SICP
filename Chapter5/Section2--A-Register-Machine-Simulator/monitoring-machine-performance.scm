...
1. to each instruction execution procedure, one must add (machine 'increment-instruction-count).

2. more elegant way: modify execute to call increment-instruction-count! after each instruction
   is executed; I use this approach.
|#
(define (make-new-machine)
  (let ((pc (make-register 'pc))
    ...
    (let ((the-ops
      ...
      (define (increment-intruction-count!)
        (set! instruction-count (+ instruction-count 1)))
      (define (reset-instruction-count!)
        (set! instruction-count 0))
      (define (print-intruction-count)
        (newline)
      ...
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ((eq? message 'print-intruction-count)
               (print-intruction-count))
...
;; Ex. 5.16
#|
There is a simple and elegant way to enact this: modify execute
|#
;; Internal definition within make-new-machine
(define (execute)
  (let ((insts (get-contents pc)))
    (if (null? insts)
        'done
        (begin
          ...
          ((instruction-execution-proc (car insts)))
          (execute)))))
;; Add dispatches to make-new-machine
((eq? message 'trace-on)
 (set! trace true))
((eq? message 'trace-off)
 (set! trace false))
#|
In fact, we can be more efficient within execute by checking trace only once.
Keep the original execute definition, and add a new definition: execute-with-trace
|#
